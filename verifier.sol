...
contract PlonkVerifier {
    ...
    uint16 constant nPublic =  25;
    uint16 constant nLagrange = 25;
    
    uint256 constant Qmx = 4876313201089979509824667013837422095656550061888541251117958086091091921176;
    uint256 constant Qmy = 11886998564469753639017302492134310049651952694650647203957404440220716607104;
    uint256 constant Qlx = 4403343471352424104793528883306973724065439822051330923955651970061361967975;
    uint256 constant Qly = 20291528045509745086791771051538928928385485206093100983381347639370257303152;
    uint256 constant Qrx = 378922330151845384529630848570907319112225807720977455462686562867597654832;
    uint256 constant Qry = 560339257290102118192493516017855240971972579167936633533561909180535988114;
    uint256 constant Qox = 19349552882453125525397550990483536677350999063470933617487713161580317619322;
    uint256 constant Qoy = 13058207513046292213908627927534155720553973535364466036303408811546381268170;
    uint256 constant Qcx = 17947487884987216037959094972647985384565168090759000840115681932538744775715;
    uint256 constant Qcy = 10005031705040185089018576178538096456672547870077695453904784178040954335730;
    uint256 constant S1x = 898451532090624982485768330607549623576761308247699547990296945984587438576;
    uint256 constant S1y = 8229341398006153730103982198970785339809334513529324517948393505032902503934;
    uint256 constant S2x = 1344315153738959911423512001371492531601800933090980034319054509774709094110;
    uint256 constant S2y = 14619543889867866822307616517214738274158775620475678668871175186431316866637;
    uint256 constant S3x = 8324903663240797085203433766583740091793789952072749523773562400053053595280;
    uint256 constant S3y = 11249903668835819134906737408143336640017990917459846377449161088015062876075;
    uint256 constant k1 = 2;
    uint256 constant k2 = 3;
    uint256 constant X2x1 = 13687756663465285158535767195535466292135144315851676371230784034123569132297;
    uint256 constant X2x2 = 11646989238832415342977209598797286327856595361328855098116174108847806320496;
    uint256 constant X2y1 = 7958059662821197306585959390455714992272772634787178539801560645790917973905;
    uint256 constant X2y2 = 2963910448408553906945753358663232874138248349341874274323413006327532488134;
    ...
    function verifyProof(bytes memory proof, uint[] memory pubSignals) public view returns (bool) {
        assembly {
            ...
            function calculateChallanges(pProof, pMem, pPublic) {
                ...
                mstore( add(pMem, 1504 ), mload( add( pPublic, 64)))
                
                mstore( add(pMem, 1536 ), mload( add( pPublic, 96)))
                
                mstore( add(pMem, 1568 ), mload( add( pPublic, 128)))
                
                mstore( add(pMem, 1600 ), mload( add( pPublic, 160)))
                
                mstore( add(pMem, 1632 ), mload( add( pPublic, 192)))
                
                mstore( add(pMem, 1664 ), mload( add( pPublic, 224)))
                
                mstore( add(pMem, 1696 ), mload( add( pPublic, 256)))
                
                mstore( add(pMem, 1728 ), mload( add( pPublic, 288)))
                
                mstore( add(pMem, 1760 ), mload( add( pPublic, 320)))
                
                mstore( add(pMem, 1792 ), mload( add( pPublic, 352)))
                
                mstore( add(pMem, 1824 ), mload( add( pPublic, 384)))
                
                mstore( add(pMem, 1856 ), mload( add( pPublic, 416)))
                
                mstore( add(pMem, 1888 ), mload( add( pPublic, 448)))
                
                mstore( add(pMem, 1920 ), mload( add( pPublic, 480)))
                
                mstore( add(pMem, 1952 ), mload( add( pPublic, 512)))
                
                mstore( add(pMem, 1984 ), mload( add( pPublic, 544)))
                
                mstore( add(pMem, 2016 ), mload( add( pPublic, 576)))
                
                mstore( add(pMem, 2048 ), mload( add( pPublic, 608)))
                
                mstore( add(pMem, 2080 ), mload( add( pPublic, 640)))
                
                mstore( add(pMem, 2112 ), mload( add( pPublic, 672)))
                
                mstore( add(pMem, 2144 ), mload( add( pPublic, 704)))
                
                mstore( add(pMem, 2176 ), mload( add( pPublic, 736)))
                
                mstore( add(pMem, 2208 ), mload( add( pPublic, 768)))
                
                mstore( add(pMem, 2240 ), mload( add( pPublic, 800)))
                
                mstore( add(pMem, 2272 ), mload( add( pProof, pA)))
                mstore( add(pMem, 2304 ), mload( add( pProof, add(pA,32))))
                mstore( add(pMem, 2336 ), mload( add( pProof, add(pA,64))))
                mstore( add(pMem, 2368 ), mload( add( pProof, add(pA,96))))
                mstore( add(pMem, 2400 ), mload( add( pProof, add(pA,128))))
                mstore( add(pMem, 2432 ), mload( add( pProof, add(pA,160))))
                
                b := mod(keccak256(add(pMem, lastMem), 992), q) 
            ...
            function calculateLagrange(pMem) {
                ...
                inverseArray(add(pMem, pZhInv), 26 )
